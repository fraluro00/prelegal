...
function confidentialityTerm(data: NDAFormData): string {
  return data.confidentialityType === 'perpetuity'
    ? 'In perpetuity'
...
### MNDA Term
${data.mndaTermType === 'expires' ? `- [x] Expires ${data.mndaTermYears} year(s) from Effective Date.` : '- [ ] Expires [...] from Effective Date.'}
${data.mndaTermType === 'until_terminated' ? '- [x] Continues until terminated in accordance with the terms of the MNDA.' : '- [ ] Continues until terminated in accordance with the terms of the MNDA.'}

### Term of Confidentiality
${data.confidentialityType === 'years' ? `- [x] ${data.confidentialityYears} year(s) from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws.` : '- [ ] [...] year(s) from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws.'}
...
**5. Term and Termination.** This MNDA commences on the **${effectiveDate}** and expires at the end of the **${term}**. Either party may terminate this MNDA for any or no reason upon written notice to the other party. The Receiving Party's obligations relating to Confidential Information will survive for the **${confidentiality}**, despite any expiration or termination of this MNDA.
